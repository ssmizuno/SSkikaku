...
@implementation FifthPageView

- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
		//背景フレームの作成
		UIImage *backGroundImage = [UIImage imageNamed:@"Base05.png"];
		...
		NSString *textPath01 = [[NSBundle mainBundle] pathForResource:@"PublishText06" ofType:@"txt"];
		...
		for (int i = 0; i < 4; i++) {
			...
			UIImage *scrollImage;
			if(i < 3){
				scrollImage = [UIImage imageNamed:[NSString stringWithFormat:@"f0%02d.png", i+1]];
			}else {
				scrollImage = [UIImage imageNamed:[NSString stringWithFormat:@"f0%02d.png", i+1]];
