...
@implementation ForthPageView

- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
		//背景フレームの作成
		UIImage *backGroundImage = [UIImage imageNamed:@"Base04.png"];
		...
		NSString *textPath01 = [[NSBundle mainBundle] pathForResource:@"PublishText05" ofType:@"txt"];
		...
		for (int i = 0; i < 5; i++) {
			...
			UIImage *scrollImage;
			if(i < 4){
				scrollImage = [UIImage imageNamed:[NSString stringWithFormat:@"t0%02d.png", i+1]];
			}else {
				scrollImage = [UIImage imageNamed:[NSString stringWithFormat:@"t0%02d.png", i+1]];
